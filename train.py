    ...
    for C in C_list:
        for solver in solver_list:
            ...
            for (key, fold) in folds_dict.items():
                val_index = X.index[ X['product_code'].isin(fold[1]) ]
                ...
                X_fold_train = X_fold_train.drop(labels=['product_code'], axis='columns')
                X_fold_val = X_fold_val.drop(labels=['product_code'], axis='columns')
            ...
            print('C = %.6f, solver = %18s, score_mean = %.6f' % (C, solver, score_mean))
...
if __name__ == '__main__':
    ...
    X_train, X_val, Y_train, Y_val = train_test_split(features, labels, random_state=109550133, train_size=TRAIN_SIZE)
    print('training...')
